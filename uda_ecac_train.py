...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss_ECAC = torch.zeros(3, device=device)  
        if RANK != -1:
        ...
        for i, ((imgs_s, targets_s, paths_s, _), (imgs_t, _, paths_t, _)) in pbar:  # batch -------------------------------------------------------------      
            ...
            with amp.autocast(enabled=cuda):
                ...
                if out.shape[0] > 0 and out_s.shape[0] > 0:
                    # get best region from target
                    region_t1_original, out1_original, best_side = get_best_region(out, imgs_t)
                    # get best region from source
                    region_s1_original, out2_original, best_side_s = get_best_region(out_s, imgs_s)
                    ...
                    transform = A.Compose([
                                        ...
                                        A.Downscale (scale_min=0.5, scale_max=0.99, interpolation=cv2.INTER_AREA, always_apply=False, p=0.5),
                    ...
                    region_t3, out3 = transform_img_bboxes(out2_original, best_side_s, region_s1_original, transform)
                    region_t4, out4 = transform_img_bboxes(out2_original, best_side_s, region_s1_original, transform)
                ...
                imgs_ECAC = imgs_concat
                out_s = torch.from_numpy(out_s) if out_s.size else torch.empty([0,7])
                b, c, h, w = imgs_ECAC.shape
                
                # create ECAC targets 

                targets_ECAC_t = out
                targets_ECAC =  targets_ECAC_t

                c_gamma_thres = 0.5
                gamma = (targets_ECAC[:, 6] > c_gamma_thres).sum() / \
                        (targets_ECAC[:, 6]).nelement()

                targets_ECAC = targets_ECAC[:,:6] # remove confidence values
                ...
                pred_ECAC = model(imgs_ECAC, pseudo=True)  # forward
                _, pred_ECAC, var_ECAC = pred_ECAC
                ...
                loss_ECAC, loss_items_ECAC = compute_loss(pred_ECAC, targets_ECAC.to(device), var_ECAC)
                '''
                '''
                #loss_ECAC, loss_items_ECAC = compute_loss(pred_ECAC, targets_ECAC.to(device), var_ECAC)
                ...
                total_loss = loss + loss_ECAC * torch.nan_to_num(gamma)
            ...
            if i < 3:
                ...
                f = save_dir / f'imgs_augblend_{epoch}_{i}_loss_{loss.clone().detach().cpu().numpy()}_loss_t_{loss_ECAC.clone().detach().cpu().numpy()}__gamma{gamma}.jpg'  # filename
                plot_images(imgs_ECAC, targets_ECAC, "", f)
            ...
            if RANK in {-1, 0}:
                ...
                mloss_ECAC = (mloss_ECAC * i + loss_items_ECAC) / (i + 1)  # update mean losses
